...
const WEDDING = {
  date: "2026-05-03T11:30:00+07:00",   // ngày giờ cưới (đếm ngược)
  calMonth: 4,                          // tháng lịch (0=Th1 … 4=Th5)
  calYear: 2026,
  calMarkDay: 3,                        // ngày đánh dấu ❤
  gallery: [
...
// ====== NHẠC NỀN ======
function playMusic() {
  bgMusic.play().then(() => musicBtn.classList.add("music-btn--playing")).catch(() => {});
}
...
(function buildCalendar() {
  ...
  const daysInMonth = new Date(WEDDING.calYear, WEDDING.calMonth + 1, 0).getDate();
  let html = "";
  for (let i = 0; i < firstDay; i++) html += `<span class="cal-empty">.</span>`;
  for (let d = 1; d <= daysInMonth; d++)
    html += `<span class="${d === WEDDING.calMarkDay ? "cal-mark" : ""}">${d}</span>`;
  grid.innerHTML = html;
})();
...
document.querySelectorAll(".tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    document.querySelectorAll(".tab-panel").forEach((p) =>
      p.classList.toggle("tab-panel--active", p.dataset.panel === key)
    );
  });
});
...
(function buildGallery() {
  ...
  grid.innerHTML = WEDDING.gallery
    .map((src) => `<figure><img src="${src}" alt="Ảnh cưới" loading="lazy" /></figure>`).join("");
  ...
  grid.addEventListener("click", (e) => {
    ...
    lbImg.src = img.src; lb.classList.add("lightbox--show");
  });
  lb.addEventListener("click", (e) => {
    if (e.target === lb || e.target.classList.contains("lightbox__close")) lb.classList.remove("lightbox--show");
  });
})();
